...
版本说明：当前源码版本为 `1.7.0 (build 30)`；最新公开 Release 仍为 `1.6.1 (build 24)`。不得用旧版本或同版本不同构建覆盖。
...
- Desktop 不可用时不再静默降级：飞书会显示具体原因，并让用户选择“重试 Desktop”“使用备用 CLI”或取消；只有主动确认后才会启动备用 CLI
- Desktop 实时同步只订阅当前 turn 的状态变化，不再回放完整历史，避免同一条飞书消息在桌面界面重复显示
- 每位用户有一张持续更新的当前状态卡，集中显示项目、Task、运行/排队状态、最近提问、最近回复和完成时间
- Mac App 显示 Codex 官方接口返回的剩余额度与重置时间；飞书通过独立的“Codex 用量”Bot 菜单打开精简卡片，不再占用当前 Task 状态卡
...
4. 点击“开启桥接”；
5. 确认 App 中的四个 Event Key 与飞书后台完全一致，然后点击飞书 Bot 菜单中的“选择 Task”。
...
- 点击 Bot 菜单“Codex 用量” → 打开独立用量卡，查看剩余百分比、重置时间并可原地刷新；所有桥接白名单用户均可查看同一台 Mac 上 Codex 账号的共享额度
