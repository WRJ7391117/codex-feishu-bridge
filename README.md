...
版本说明：当前源码和本机构建为 `1.11.13 (build 93)`，支持 `macOS 13` 或更高版本。不得用旧版本或同版本不同构建覆盖。
...
- 每位授权用户可独立订阅最多 20 个有权访问的 Task；这些 Task 在 Codex Desktop 出现人工门时自动推送操作卡片，完成新运行后自动推送结果，订阅前的历史结果不会补发
...
桥接会在当前 macOS 用户登录后自动启动，但不会自动弹出 App 窗口。首页会区分“正在启动”和“已开启”；三类飞书事件监听全部连接后才显示为已开启。“停止本次运行…”只停止当前登录会话，自动启动开关保持不变；只有明确关闭“登录后自动启动”开关，后续登录才不会自动恢复。
...
- 点击 Bot 菜单“桌面task”→“订阅桌面 Task” → 按项目筛选并逐个订阅或取消订阅；订阅不改变当前 Task，Desktop 以后出现的人工门和完成结果会自动推送，收到结果后当前 Task 跟随该结果以便直接追问
...
- 点击 Bot 菜单“提示灯”→“我的提示灯” → 查看本人名下提示灯、在线状态、默认灯、当前中继、最后逻辑状态和每灯独立的 Task 白名单；安装包已内置 Universal PromLight Helper，无需另装 PromLight App；设备连接和用户归属在 Mac App 首页完成，飞书卡片再分为“提示灯关联哪些 Task”和“提示灯设备设置”两条路径；“解除与 Bridge 的绑定”会停止提醒并清除关联列表，但不会断开 macOS 蓝牙
