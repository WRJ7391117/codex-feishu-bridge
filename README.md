...
版本说明：当前源码发布版本为 `1.5.3 (build 15)`，包含并取代 `1.5.2 (build 14)`、`1.5.1 (build 13)` 以及原 Ori One 计划中的 `1.4.3` 桥接修复；不得用旧版本或同版本不同构建覆盖。

## 能力

- 飞书机器人菜单分别提供“选择 Task”“新建 Task”“归档 Task”，各自打开职责单一的卡片
...
- 可从独立卡片选择项目后新建 Task，或二次确认归档当前 Task
...
- 同一 Task 运行中收到的新消息会按顺序排队，显示位置，并可从卡片取消
...
- App 启动时自动检查 GitHub Releases，新版本可从控制中心直接打开下载
...
5. 在机器人菜单中添加三个独立按钮，动作都选择“推送事件”：
   - “选择 Task” → Event Key `select_task`
   - “新建 Task” → Event Key `new_task`
   - “归档 Task” → Event Key `archive_task`
...
4. 点击“开启桥接”；
5. 确认 App 中的三个 Event Key 与飞书后台完全一致，然后点击飞书 Bot 菜单中的“选择 Task”。
...
- 点击 Bot 菜单“新建 Task” → 在独立卡片中选择项目并确认 → 发送 Task 标题 → Bot 创建并自动选择该 Task
- 点击 Bot 菜单“归档 Task” → 独立卡片显示当前 Task → 二次确认后归档；可在 Desktop 恢复
...
`1.5.3 (build 15)` 已覆盖已有 Task 续聊、附件、实时进度、队列/停止、一次性授权、新建 Task、搜索/分页和归档，并保证 workflow 决策先持久化、再执行任何辅助去重。Desktop 的“置顶”目前没有可供第三方稳定调用的 Codex app-server 方法，因此桥接不直接修改 Codex SQLite 或 Electron 私有状态；置顶仍在 Desktop 中操作。飞书真实点击、文件/音频、授权回调和 workflow 决策往返仍需要在目标租户中做端到端验收，本地自测不能替代该证据。
