...
版本说明：当前源码发布版本为 `1.5.10 (build 22)`，包含并取代 `1.5.9 (build 21)`、`1.5.8 (build 20)`、仅在本机验证的 `1.5.4`—`1.5.7` 以及已发布的 `1.5.3 (build 15)`；不得用旧版本或同版本不同构建覆盖。
...
- 运行、排队、授权和结果卡片统一显示绿色“当前 Task”标识，并分行显示项目和 Task 标题
...
- 点击 Bot 菜单“选择 Task” →“查看已归档 Task”→ 选择并确认恢复；恢复后自动成为当前 Task
...
`1.5.10 (build 22)` 已覆盖已有 Task 续聊、附件、实时进度、队列/停止、一次性授权、新建 Task、搜索/分页、归档及恢复，并统一标记飞书中的当前 Task，同时保证 workflow 决策先持久化、再执行任何辅助去重。Desktop 的“置顶”目前没有可供第三方稳定调用的 Codex app-server 方法，因此桥接不直接修改 Codex SQLite 或 Electron 私有状态；置顶仍在 Desktop 中操作。飞书真实点击、文件/音频、授权回调和 workflow 决策往返仍需要在目标租户中做端到端验收，本地自测不能替代该证据。
