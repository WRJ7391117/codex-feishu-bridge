...
版本说明：当前源码发布版本为 `1.5.8 (build 20)`，包含并取代仅在本机验证的 `1.5.4`—`1.5.7` 以及已发布的 `1.5.3 (build 15)`；不得用旧版本或同版本不同构建覆盖。
...
`1.5.8 (build 20)` 已覆盖已有 Task 续聊、附件、实时进度、队列/停止、一次性授权、新建 Task、搜索/分页、归档及恢复，并保证 workflow 决策先持久化、再执行任何辅助去重。Desktop 的“置顶”目前没有可供第三方稳定调用的 Codex app-server 方法，因此桥接不直接修改 Codex SQLite 或 Electron 私有状态；置顶仍在 Desktop 中操作。飞书真实点击、文件/音频、授权回调和 workflow 决策往返仍需要在目标租户中做端到端验收，本地自测不能替代该证据。
