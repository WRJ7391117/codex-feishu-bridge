...
状态：Mac 桌面路径已实现；手机/Pad 不作为当前产品的持续中继。
...
Codex 仍运行在共享 Mac。飞书只提供身份、配置和通知入口。当前有效硬件路径是 PromLight 与运行 Bridge 的 Mac 建立 BLE/HID 连接；手机或 Pad 不是业务绑定对象，也不承担持续中继。灯归属和 Task 订阅由飞书 `open_id` 保持。
...
灯离线时保留最后逻辑状态，并明确显示“提示灯离线”。内置 Helper 的 HID 写入成功记录为 `submitted`，只表示命令已经提交给设备，不等于灯效 read-back；旧 HTTP 兼容 relay 的设备 ACK 记录为 `acknowledged`，同样不等于独立灯效验证。`no-device`、写入失败、超时或网络错误记录为离线/unknown，并由单一合并 worker 按有界退避重试；不确定 Task 状态不会转换为绿色命令。

## 桌面 relay

DeepOri Bridge 安装包内置最小 Universal `promlight-helper`，通过 macOS IOKit 完成：

- 按 PromLight VID/PID 和 HID usage 枚举在线设备；
- 使用设备稳定 SerialNumber 作为仅保存在本机状态中的 opaque relay reference；
- 把 `idle`、`running`、`human_gate`、`error` 编译为固定 5E5E HID 帧；
- 使用 HID output report ID `2` 提交命令，不启动额外 daemon、端口或定时轮询。

实现依据公开的 [PromLight](https://github.com/piaomiaoguying/PromLight) 5E5E 协议与 MIT 声明独立编写，没有复制上游 `promlightd`、Claude Hook、HTTP server、宏或 session 管理代码。旧 PromLight App `0.2.3` 仅保留为迁移兼容回退，其本机 API 为：
...
当前已验证组合为 PromLight 硬件（设备报告版本 `0.1.3`、release `19`）与 Bridge 内置 Helper `1`，连接通道为 Mac BLE/HID。Bridge App 首页直接展示多灯发现、选择用户、命名和绑定流程，并显示连接设备实际报告的产品与版本。其他硬件型号或版本未完成真实验证前，不宣称兼容。

其他 Mac 安装 DeepOri Bridge 后会同时获得 Universal Helper，无需再安装 PromLight App、Claude Hook 或额外驱动服务。现有 PromLight App 可暂时保留作为兼容回退；Bridge 不复制、重新签名或分发该第三方 App。
...
官方运行机制同时说明：移动端小程序进入后台后一般最多保留约 5 分钟，也可能因系统资源或 iOS 内存告警提前销毁。因此飞书小程序不能承担持续 Task 提醒 relay，当前产品不提供手机/Pad 配对入口。

当前公开资料和本机验证覆盖 macOS BLE HID/5E5E 路径，不构成手机原生 BLE GATT 持续中继证据。因此不实现或宣称手机真机闭环，飞书卡片也不显示移动配对入口。
...
灯归属、默认灯、每灯 Task 白名单、最后逻辑状态和投递语义存入现有原子化 `state.json`，重启后恢复。取消订阅、Task 归档、权限撤销或解绑时立即停止后续 Task 驱动；解绑或撤权只有在 idle 命令成功提交或收到旧 relay ACK 后才删除绑定，离线时保留“解绑待收口”以便恢复重试。最后一个订阅被移除时收口到 idle 后不再接收 Task 事件。实现复用现有 rollout 事件观察节拍，不新增 10/30 分钟空轮询或云服务。
