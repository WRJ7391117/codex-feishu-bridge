...
| --- | --- | --- | --- |
| Source tests | macOS arm64 development host | 252 Python tests pass | Pass |
| App compiler | macOS 13 deployment target | Swift type-check passes | Pass |
...
| Runtime | current Mac existing install | previous bridge runtime remains healthy; new alpha build was not installed over it | Existing version only |
...
| GitHub CI | macOS runner | workflow YAML parses locally; remote run requires push | Not run remotely |
| GitHub tag Release | public repository | tag/version gate and artifact upload workflow added | Not run remotely |
