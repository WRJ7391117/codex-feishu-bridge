...
- “接续桌面” shows the wrong Task: verify the intended Task is within the user's project access. The bridge prefers that user's selected running Task, otherwise the most recently used running Task, and otherwise the most recently used completed Task; the card always names the chosen project and Task before any result is returned.
- “接续桌面” remains running after Desktop completed: verify that the Task rollout still exists and is appendable, inspect `desktop sync loop failed`, and do not delete `state.json`; the subscription cursor is intentionally persisted across bridge restarts.
...
- A single `no-client-found`, pre-submit Desktop connection failure, or read-only Task-database `OperationalError` is retried briefly. The bridge never retries after a turn request may already have reached Desktop; inspect the Task instead of replaying an uncertain submission.
...
- Diagnostics intentionally retain the bundled `lark-cli 1.0.89-codex-feishu.2`. They suppress only the false same-base `1.0.89` update warning that would remove the bridge patch, while preserving a warning for any genuinely newer upstream base version.
