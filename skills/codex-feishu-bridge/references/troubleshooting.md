...
- Project/Task selection succeeds but Feishu shows `108002`: verify the installed runtime is using the bundled `lark-cli 1.0.92-codex-feishu.3`. Official v1.0.92 routes the card action through the generic custom-event handler and does not return the card-action response Feishu expects within three seconds.
...
- Bot menu click does nothing: each clickable submenu must use “push event” and its Event Key must match the corresponding configured key. “Task 管理” groups `current_task`, `select_task`, `new_task`, and `archive_task`. “桌面task” groups `task_subscriptions`, `sync_desktop`, and `sync_desktop_switch`. “模型设置” groups `task_settings`, `compact_task_context`, and `codex_usage`. “提示灯” groups `promlight` and `promlight_legend`.
...
- Diagnostics intentionally retain the bundled `lark-cli 1.0.92-codex-feishu.3`. They suppress only the false same-base `1.0.92` update warning that would remove the bridge patch, while preserving a warning for any genuinely newer upstream base version.
...
- A final text reply that exhausts immediate retries is stored in `state.json` and retried with the original idempotency key. Failed queue cards and the latest failed patch for each progress card are also persisted; successful recovery removes them without rerunning Codex. An old queue card is discarded if its input has already started. Local images up to 20 MB are copied into the private retry spool, which has a 100 MB total limit. When that spool is full, the bridge preserves older pending results, rejects the new cache item, and tells the user to view it in Codex Desktop.
...
- Codex result text arrives but result audio does not: confirm the final reply contains an explicit absolute local audio link. Opus and Ogg Opus use `--audio`; MP3, WAV, M4A, AAC, and FLAC intentionally use `--file`. Inspect the bridge log for `audio reply failed` or `audio_reply queued`; a queued audio item must retry as audio rather than being reclassified as a document.
