...
- A final text reply that exhausts immediate retries is stored in `state.json` and retried with the original idempotency key. Successful recovery removes it from the queue and sends a short delayed-delivery explanation; running-status and local image replies are not persisted.
